	...
	private void Move(){
		...
		if (holdingBox)
		{
			...
			if(transform.Find("Box")){
				transform.Find ("Box").GetComponent<Rigidbody>().velocity = new Vector3(horizontalDirection * movementSpeed * speedModifier, verticalDirection * movementSpeed * speedModifier, 0f);
			} else if(transform.Find("Box-Invisible")) {
				transform.Find ("Box-Invisible").GetComponent<Rigidbody>().velocity = new Vector3(horizontalDirection * movementSpeed * speedModifier, verticalDirection * movementSpeed * speedModifier, 0f);
			} else {
